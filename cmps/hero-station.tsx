...
export function HeroStation({ station }: Props) {
    ...
    return (
        <div key={station._id} className="flex items-center justify-between rounded bg-[#2c2c2c] hover:bg-[#333]"
            ...
            <button className={`transition-all ease-in flex justify-center items-center mr-2 rounded-full bg-[#1ed760] w-12 h-12 ${isHovered ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-1.5'}`}>
